  ...
  vector[L_color] p ; // for storing log-probabilities
    ...
    vector[N_color] logKappaEffect; //intermediate value
		...
		for(this_id in 1:N_toj){
			...
			+ population_pss_attention_initial_bias_judgement_type_interaction_effect_mean * condition_initial_bias[this_id] * condition_judgement_type[this_id] )/2 ;
    	...
    	// two-way interactions
			...
			// three-way interactions
			+ population_logjnd_attention_initial_bias_judgement_type_interaction_effect_mean * condition_initial_bias[this_id] * condition_judgement_type[this_id] )/2;
		}
